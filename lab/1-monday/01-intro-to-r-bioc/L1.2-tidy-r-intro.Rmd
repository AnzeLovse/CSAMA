...
fname <- "BRFSS-subset.csv"   ## BRFSS-subset.csv
...
```{r}
all_data <- read_tsv(file = "ALLphenoData.tsv", )
all_data

all_data <- all_data |> 
  mutate(sex = factor(sex), mol.biol = factor(mol.biol))
```
...
2. Use `filter()` to create a subset of the data consisting only of
   ...
   base _R_. Likewise, create a subset with only "BCR/ABL" and "NEG"
   in the `mol.biol` column.

```{r}
all_data |>
  filter(sex == "F", mol.biol %in% c("BCR/ABL", "NEG"))
```
...
```{r}
all_data |>
  mutate(BT = factor(substr(BT, 1, 1)))
```
...
```{r}
t_test <- function(data, formula, ...) {
    broom::tidy(t.test(formula, data, ...))
}
